...
module.exports = class Battler {
  ...
  async chooseAction(msg) {
    ...
    let content = `
      ...
      'or',
    )}? You have **${this.mp}** MP. **${this.battle.user.user.tag}:** ${
      this.battle.user.hp
    } HP **${this.battle.opponent.user.tag}:** ${this.battle.opponent.hp} HP`;
    ...
    const filter = (res) => {
      ...
      if (res.author.id === this.user.id && choices.includes(choice)) {
        if (
          (choice === 'special' && !this.canSpecial) ||
          (choice === 'cure' && !this.canHeal)
        ) {
          msg.send("You don't have enough MP for that!").catch(() => null);
          return false;
        }
        if (choice === 'final' && !this.canFinal) {
          msg
            .send(
              'You must have under 100 HP and over 100 MP. Final can only be used once!',
            )
            .catch(() => null);
          return false;
        }
