...
module.exports = class Player {
  ...
  async dmRound() {
    const valid = Array.from(
      this.game.players.filter(p => p.role !== this.role).values()
    );
    ...
    if (this.role === 'detective') {
      ...
      await this.user.send(
        isMafia ? 'Yes, they are a Mafioso.' : 'No, they are not a Mafioso.'
      );
    } else {
      await this.user.send(
        `**${this.game.players.get(choice).user.tag}** is your choice...`
      );
    }
    return choice;
  }
