...
module.exports = class Game {
  ...
  async generate(list) {
    ...
    for (const user of list) {
      try {
        await user.send(
          `You are ${roles[i] === 'detective' ? 'the' : 'a part of the'} **${
            roles[i]
          }**.`
        );
      } catch (err) {
  ...
  playAudio(id) {
    this.dispatcher = this.connection.play(
      path.join(__dirname, '..', '..', 'sounds', 'mafia', `${id}.mp3`),
      {
        volume: 2,
      }
    );
  ...
  getHanged(votes, playersArr) {
    ...
    for (const vote of votes.values()) {
      const player = this.players.get(
        playersArr[Number.parseInt(vote.content, 10) - 1].id
      );
  ...
  get shouldEnd() {
    console.log('Players Left:', this.players.size);
    console.log(
      'Mafia Left:',
      this.players.some(p => p.role === 'mafia')
    );
