...
module.exports = class extends Command {
  ...
  init() {
    ...
    this.avatar = (size) =>
      this.client.user.avatarURL({ format: 'png', size: size });
  ...
  finish(data, stopwatch, format) {
    ...
    return this.msg.channel.sendFile(
      buffer,
      `commands_${hash}.${format}`,
      duration,
    );
  }
  ...
  async markdown(msg) {
    const { commands, categories, stopwatch } = await this.buildCommands(
      'markdown',
      msg,
    );
    const markdown = [];
...
<h1>${esc(this.username)}</h1>
    ...
    for (let cat = 0; cat < categories.length; cat++) {
      ...
      if (category) {
        html += `<h2 class="category-header">${esc(categories[cat])} / ${
          category.length
        } Commands</h2>`;
      }
      ...
      for (let subCat = 0; subCat < subCategories.length; subCat++) {
        ...
        if (subCategories.length > 1)
          html += `<h3 class="subcategory-header">${esc(
            subCategories[subCat],
          )}</h3>`;
...
<tr><th>Command</th><th>Aliases</th><th>Description</th></tr>
  ...
  async plaintext(msg) {
    const {
      ...
      longest,
      stopwatch,
    } = await this.buildCommands('plaintext', msg);
    const plaintext = [];
    ...
    for (let cat = 0; cat < categories.length; cat++) {
      ...
      for (let subCat = 0; subCat < subCategories.length; subCat++) {
        if (subCategories.length > 1)
          plaintext.push(`${subCategories[subCat]}\n`);
        plaintext.push(
          `${commands[categories[cat]][subCategories[subCat]]
            .map(
              (cmd) =>
                `${this.prefix + cmd.name.padEnd(longest)}: ${cmd.description}`,
            )
            .join('\n')}`,
        );
        plaintext.push('\n');
